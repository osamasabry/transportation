...
<head>
    ...
    <title>Trust - Search Orders</title>
...
<body class="request_quote">
   ...
   <header>
    ...
    <!--================================
        START QUOTE AREA
    =================================-->
    <section style="padding-bottom:50px;" class="get_quote section_padding reveal animated" data-delay="0.2s" data-anim="fadeInUpShort">
        <!-- container starts -->
        <div class="container">
            <!-- row starts -->
            <div class="row">
                ...
                <div class="col-md-10 col-sm-10 col-sm-offset-1 col-md-offset-1">
                    <div class="quote_requ_wrapper">
                        <div class="section_title title_center">
                            <div class="title"><h2>Result of search</h2></div>
                        </div>
            ...
            <?php
                ...
                if($field == 'id'){
                    if($format=="Equal")
                        $sql=("SELECT * FROM orders WHERE $field like '$value'");
                    ...
                    if($format=="Equal")
                        $sql=(" SELECT * FROM orders where emp_id in ( SELECT id FROM users WHERE name like '$value')");
                    elseif($format=="Begin With")
                        $sql=("SELECT * FROM orders where emp_id in ( SELECT id FROM users WHERE name like '$value%')");
                    elseif($format=="End With")
                        $sql=("SELECT * FROM orders where emp_id in ( SELECT id FROM users WHERE name like '%$value')");
                    ...
                    if($format=="Equal")
                        $sql=(" SELECT * FROM orders where driver_id in ( SELECT id FROM users WHERE name like '$value')");
                    elseif($format=="Begin With")
                        $sql=("SELECT * FROM orders where driver_id in ( SELECT id FROM users WHERE name like '$value%')");
                    elseif($format=="End With")
                        $sql=("SELECT * FROM orders where driver_id in ( SELECT id FROM users WHERE name like '%$value')");
                ...
                if($n==0){
                    ...
                    echo "<table class=\"responstable\"><tr>
                            <th >OrderID</th>
                            <th >OrderName</th>
                            <th >OrderEmail</th>
                            <th >OrderPhone</th>
                    ...
                    for($i=0;$i<$n;$i++){
                        ...
                        $name=mysqli_result($result,$i,"customer_name");
                        $phone=mysqli_result($result,$i,"phone");
                        $email=mysqli_result($result,$i,"email");
                        $date=mysqli_result($result,$i,"from_date").'/'.mysqli_result($result,$i,"to_date");
                        $accepted=(mysqli_result($result,$i,"accepted") == true)? 'True' : 'False';

                        // From place
                        $sql2=("SELECT name FROM governorates where id =".mysqli_result($result,$i,"from_place"));
                        $sql2Res=mysqli_query($link,$sql2);
                        $from = mysqli_fetch_assoc($sql2Res)['name'];
                        // To place
                        $sql3=("SELECT name FROM governorates where id = ".mysqli_result($result,$i,"to_place"));
                        $sql3Res=mysqli_query($link,$sql3);
                        $to = mysqli_fetch_assoc($sql3Res)['name'];

                        $sql4=("SELECT name FROM users where id = ".mysqli_result($result,$i,"emp_id"));
                        ...
                        $sql5=("SELECT name FROM users where id = ".mysqli_result($result,$i,"driver_id"));
                        ...
                        $sql6=("SELECT type FROM trucks where id = ".mysqli_result($result,$i,"truck_id"));
